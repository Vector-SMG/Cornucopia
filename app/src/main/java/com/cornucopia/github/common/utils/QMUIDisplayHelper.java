...
public class QMUIDisplayHelper {
  ...
  public static int getUsefulScreenWidth(Context context, boolean hasNotch) {
    ...
    if (isLandscape) {
      ...
      if (QMUIDeviceHelper.isHuawei() && !QMUIDisplayHelper.huaweiIsNotchSetToShowInSetting(
          context)) {
  ...
  public static boolean huaweiIsNotchSetToShowInSetting(Context context) {
    ...
    // 1: 隐藏显示区域
    int result =
        Settings.Secure.getInt(context.getContentResolver(), HUAWAI_DISPLAY_NOTCH_STATUS, 0);
    return result == 0;
  }

  @TargetApi(17)
  public static boolean xiaomiIsNotchSetToShowInSetting(Context context) {
    return Settings.Global.getInt(context.getContentResolver(), XIAOMI_DISPLAY_NOTCH_STATUS, 0)
        == 0;
  }
}
